...
public class AddStudentActivity extends AppCompatActivity {
    ...
    private void loadStudentInfo(String studentId) {
        ...
        if (cursor.moveToFirst()) {
            ...
            // Không hiển thị lại mật khẩu/hash đang lưu. Để trống nghĩa là giữ nguyên mật khẩu cũ.
            edtStudentPassword.setText("");
            edtStudentPassword.setHint("Để trống nếu không đổi mật khẩu");
    ...
    private void updateStudent() {
        ...
        if (!studentPassword.isEmpty()) {
            studentValues.put(
                    ...
                    dbHelper.hashStudentPasswordForStorage(studentPassword)
            );
        }
...
// Tránh làm mất Face ID cũ.
    ...
    private void saveStudent() {
        ...
        try (Cursor cursor = db.rawQuery(
            ...
            studentExists = cursor.moveToFirst();
        }

        if (!studentExists && studentPassword.isEmpty()) {
            Toast.makeText(this, "Vui lòng nhập mật khẩu sinh viên", Toast.LENGTH_SHORT).show();
            return;
        }
        if (!studentExists
                && (savedFaceEmbeddingJson == null || savedFaceEmbeddingJson.isEmpty())) {
            Toast.makeText(
                    this,
                    "Vui lòng thêm ảnh khuôn mặt sinh viên trước khi lưu",
                    Toast.LENGTH_SHORT
            ).show();
            return;
        }
        if (dbHelper.isStudentInSubject(studentId, subjectId)) {
            Toast.makeText(
                    this,
                    "Sinh viên đã được thêm vào môn học này",
                    Toast.LENGTH_SHORT
            ).show();
            return;
        }
        ...
        try {
            if (!studentExists) {
                ...
                long studentResult = db.insert(
                        DatabaseHelper.TABLE_STUDENTS,
                        null,
                        studentValues
                );
                if (studentResult == -1) {
                    throw new IllegalStateException("Không thể tạo sinh viên");
                }
            }
            ...
            if (updatedRows == 0) {
                long enrollmentResult = db.insert(
                        DatabaseHelper.TABLE_ENROLLMENTS,
                        null,
                        enrollmentValues
                );
                if (enrollmentResult == -1) {
                    throw new IllegalStateException("Không thể thêm sinh viên vào môn học");
                }
            }
            ...
            Toast.makeText(
                    this,
                    "Thêm sinh viên thất bại. Dữ liệu chưa bị thay đổi",
                    Toast.LENGTH_LONG
            ).show();
        } finally {
            db.endTransaction();
